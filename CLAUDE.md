...
- **`/gestione`**: pannello privato (noindex, solo italiano) con galleria cliccabile delle opere, filtri (Tutte/Con originale/Con stampe/Vendute-esaurite) e badge di stato; ogni card apre la scheda Keystatic corrispondente (`/keystatic/collection/artworks/item/<slug>`). Non linkato dal sito pubblico.
- **Keystatic**: menu laterale raggruppato in "Galleria" (opere) e "Sito" (impostazioni) via `ui.navigation` in `keystatic.config.ts`.
