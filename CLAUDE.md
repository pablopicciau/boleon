...
- **Stampe in vari formati** (`printSizes` sullo schema opera): ogni opera può offrire, oltre all'originale, stampe fine art in formati con prezzo/stock indipendenti. Id carrello: `slug` per il pezzo base, `slug::formato` per una stampa (separatore in `PRINT_ID_SEPARATOR`). `catalogEntries()` in `src/lib/artworks.ts` è l'unica fonte di verità per varianti/prezzi, usata sia dal carrello sia da `checkout.ts`.
