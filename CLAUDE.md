...
- **Hero spotlight** (`src/components/HeroSpotlight.astro`): hero a schermo intero in home con maschera radiale che segue il cursore e rivela una seconda opera (stile ispirato a motionsites.ai/Lithos, adattato: vanilla JS + maschera CSS, niente React/canvas). Immagini scelte in `HomePage.astro` (`heroBase`/`heroReveal`) — attualmente segnaposto, da sostituire con i disegni reali dell'artista. Animazioni in `global.css` con supporto `prefers-reduced-motion`.
